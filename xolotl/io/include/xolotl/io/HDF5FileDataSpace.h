...
template <uint32_t Rank>
HDF5File::SimpleDataSpace<Rank>::SimpleDataSpace(
	const Dimensions& _dims, const Dimensions& _maxDims) :
	...
	DataSpace(H5Aget_space(attr.getId()))
{
	if (getId() < 0) {
		std::ostringstream estr;
		estr << "Failed to create simple data space for attribute "
			 << attr.getName();
		throw HDF5Exception(estr.str());
	}

	// Retrieve the dimensions.
	auto nDims = H5Sget_simple_extent_dims(getId(), dims.data(), nullptr);
	if (nDims < 0) {
		std::ostringstream estr;
		estr << "Unable to obtain dimensions for data space for attribute "
			 << attr.getName();
		throw HDF5Exception(estr.str());
	}
	assert(nDims == Rank);
	...
	DataSpace(H5Dget_space(dsb.getId()))
{
	if (getId() < 0) {
		std::ostringstream estr;
		estr << "Failed to construct simple data space for dataset "
			 << dsb.getName();
		throw HDF5Exception(estr.str());
	}
	...
	if (nDims < 0) {
		std::ostringstream estr;
		estr << "Unable to obtain dimensions for data space for dataset "
			 << dsb.getName();
		throw HDF5Exception(estr.str());
	}
	assert(nDims == Rank);
...
{
	auto status = H5Sset_extent_simple(getId(), Rank, _dims.data(), nullptr);
	if (status < 0) {
		std::ostringstream estr;
