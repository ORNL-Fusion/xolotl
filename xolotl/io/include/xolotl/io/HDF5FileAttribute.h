...
{
template <typename T>
HDF5File::Attribute<T>::Attribute(
	const HDF5Object& target, std::string attrName, const DataSpace& ds) :
	AttributeBase(target, attrName)
{
	setId(H5Acreate(target.getId(), getName().c_str(), TypeInFile<T>().getId(),
...
template <typename T>
HDF5File::Attribute<std::vector<T>>::Attribute(
	const HDF5Object& target, std::string attrName, const DataSpace& ds) :
	AttributeBase(target, attrName)
{
	TypeInFile<T> ftype;
	setTo(H5Acreate(target.getId(), getName().c_str(), ftype.getId(),
		ds.getId(), H5P_DEFAULT, H5P_DEFAULT));
	if (getId() < 0) {
		std::ostringstream estr;
		estr << "Unable to create attribute " << getName() << " on group "
...
template <typename T>
void
HDF5File::Attribute<std::vector<T>>::setTo(const std::vector<T>& value) const
{
...
template <typename T>
std::vector<T>
HDF5File::Attribute<std::vector<T>>::get(void) const
{
...
template <typename T>
void
HDF5File::Attribute<std::vector<std::vector<T>>>::setTo(
	const std::vector<std::vector<T>>& value) const
{
	...
	for (auto i : boost::counting_range<uint32_t>(0, nVectors)) {
		vlData[i].p =
			const_cast<void*>(static_cast<const void*>(value[i].data()));
		vlData[i].len = value[i].size();
	}

	// Write the data to the attribute in the file.
	TypeInMemory<std::vector<T>> memType;
	auto status = H5Awrite(getId(), memType.getId(), vlData.data());
	if (status < 0) {
		std::ostringstream estr;
		estr << "Failed to write variable length vector data to attribute "
			 << getName();
...
HDF5File::Attribute<std::vector<std::vector<T>>>::get(void) const
{
	...
	std::vector<hvl_t> vlData(nVectors);
	TypeInMemory<std::vector<T>> memType;
	auto status = H5Aread(getId(), memType.getId(), vlData.data());
	if (status < 0) {
		std::ostringstream estr;
		estr << "Failed to read variable length vector data from attribute "
			 << getName();
		throw HDF5Exception(estr.str());
	}
...
template <typename T>
HDF5File::ExtentsAttribute<T>::ExtentsAttribute(
	const HDF5Object& target, std::string attrName, const DataSpace& ds) :
	AttributeBase(target, attrName)
{
	TypeInFile<T> ftype;
	id = H5Acreate(target.getId(), name.c_str(), ftype.getId(), ds.getId(),
		H5P_DEFAULT, H5P_DEFAULT);
	if (id < 0) {
		std::ostringstream estr;
		estr << "Unable to create attribute " << name << " on group "
			 << target.getName();
		throw HDF5Exception(estr.str());
	}
}
	...
	const std::vector<std::vector<std::array<T, 2>>>& value) const
{
	...
	// Write the data to the attribute in the file.
	TypeInMemory<T> memType;
	auto status = H5Awrite(id, memType.getId(), flatData.data());
	if (status < 0) {
		std::ostringstream estr;
...
HDF5File::ExtentsAttribute<T>::get(void) const
{
	...
	auto nValues = std::accumulate(
		dims.begin(), dims.end(), 1, std::multiplies<hsize_t>());
	...
	// Read the data from the file.
	TypeInMemory<T> memType;
	auto status = H5Aread(id, memType.getId(), flatData.data());
	if (status < 0) {
		std::ostringstream estr;
	...
	// Release memory allocated by the HDF5 library.
	H5Dvlen_reclaim(
		memType.getId(), dspace.getId(), H5P_DEFAULT, flatData.data());
