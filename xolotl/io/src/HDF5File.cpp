...
HDF5File::toHDF5AccessMode(AccessMode mode)
{
	...
	default:
		throw HDF5Exception(
			"Unrecognized file access mode given when opening/creating file");
	}
	return ret;
}

herr_t
HDF5File::BuildCurrStackLevelErrorString(
...
HDF5File::BuildHDF5ErrorString(void)
{
	...
	auto ok = H5Ewalk(
		H5E_DEFAULT, H5E_WALK_UPWARD, BuildCurrStackLevelErrorString, &estr);
	if (ok < 0) {
...
void
HDF5File::Open(fs::path _path, AccessMode _mode, MPI_Comm _comm, bool par)
{
	...
	if ((_mode == AccessMode::OpenReadOnly) or
		...
		setId(H5Fcreate(_path.string().c_str(), hdf5Mode,
