...
PetscMonitor0D::setup(int loop)
{
	...
	PetscBool flagCheck, flag1DPlot, flagBubble, flagStatus, flagAlloy,
		flagXeRetention, flagLargest, flagZr, flagRPV;
	...
	// Check the option -rpv
	PetscCallVoid(PetscOptionsHasName(NULL, NULL, "-rpv", &flagRPV));
	...
	// Set the monitor to output data for RPV
	if (flagRPV) {
		_solverHandler->getNetwork().writeMonitorOutputHeader();

		// computeRPV will be called at each timestep
		PetscCallVoid(TSMonitorSet(_ts, monitor::computeRPV, this, nullptr));
	}
