	...
	TS ts, PetscInt timestep, PetscReal time, Vec solution, void* ictx)
{
    ...
    /*
    // Only output every 5 timesteps:
    if (timestep%1 !=0) PetscFunctionReturn(0);
    ...
    // The vector of ids for which clusters to output
    std::vector<IdType> iClusterIds0D;
    iClusterIds0D.clear();
	...
	while (iClusterExists) {
		...
		if (clusterId != NetworkType::invalidIndex()) {
			iClusterIds0D.push_back(clusterId);
			iSize++;
		}
		else
			iClusterExists = false;
    
        //Only add IDs to a size of 9
        if (iSize == 9) 
            iClusterExists = false;
	}
    ...
    //std::cout << iClusterIds0D.size();

     // Output the data

     outputFile << timestep << " " << time << " ";
     for (auto i = 0; i < iClusterIds0D.size(); ++i) {
         outputFile << gridPointSolution[iClusterIds0D[i]] << " ";
     }
     outputFile << std::endl;
...
// Initial declarations
    ...
    else if (timestep%500000 !=0 ) {PetscFunctionReturn(0);}
    //if (timestep%10 !=0 ) PetscFunctionReturn(0);
