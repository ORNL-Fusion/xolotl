	...
	TS ts, PetscInt timestep, PetscReal time, Vec solution, void* ictx)
{
	...
	PetscCall(static_cast<PetscMonitor3D*>(ictx)->monitorSurfaceXY(
...
{
	PetscFunctionBeginUser;
	PetscCall(static_cast<PetscMonitor3D*>(ictx)->monitorSurfaceXZ(
		ts, timestep, time, solution));
	PetscFunctionReturn(0);
}

void
PetscMonitor3D::setup(int loop)
{
	_loopNumber = loop;
	...
	PetscBool flagCheck, flagPerf, flagHeRetention, flagXeRetention, flagStatus,
		flag2DXYPlot, flag2DXZPlot, flagLargest, flagPassData;
	...
	PetscInt Mx, My, Mz;
	PetscCallVoid(DMDAGetInfo(da, PETSC_IGNORE, &Mx, &My, &Mz, PETSC_IGNORE,
	...
	if (flagXeRetention) {
		...
		PetscInt xm, ym, zm;
		PetscCallVoid(DMDAGetCorners(da, NULL, NULL, NULL, &xm, &ym, &zm));
	...
	if (flagXeRetention) {
		...
		_solverHandler->createLocalDefects(1, xm, ym, zm);
	...
	TS ts, PetscInt timestep, PetscReal time, Vec solution)
{
	...
	if (procId == 0) {
		...
		PetscInt Mx, My, Mz;
		PetscCall(DMDAGetInfo(da, PETSC_IGNORE, &Mx, &My, &Mz, PETSC_IGNORE,
...
PetscErrorCode
PetscMonitor3D::computePassData(
	TS ts, PetscInt timestep, PetscReal time, Vec solution)
{
	// Initial declarations
	IdType xs, xm, Mx, ys, ym, My, zs, zm, Mz;
	...
	for (auto const& pair : gbVector) {
		...
		if (xi >= xs && xi < xs + xm && yj >= ys && yj < ys + ym && zk >= zs &&
			...
			double previousDefectFlux =
				localDefects[xi - xs][yj - ys][zk - zs][1];
			globalDefectFlux +=
				previousDefectFlux * (grid[xi + 1] - grid[xi]) * hy * hz;
			// Set the amount in the vector we keep
			_solverHandler->setLocalDefectRate(
				previousDefectFlux * dt, 0, xi - xs, yj - ys, zk - zs);
		}
	}
	...
	if (procId == 0) {
		...
		// Print the result
		XOLOTL_LOG << "Xenon GB = " << nXenon / surface << "\n\n";
	}
						...
						double prob = prefactor *
							...
							std::min(1.0,
								exp(-(distance - depthParam) /
									(depthParam * 2.0)));
						double test =
							_solverHandler->getRNG().GetRandomDouble();
	...
	PetscReal time, Vec solution, PetscBool)
{
	...
	for (auto i = 0; i < _depthPositions.size(); i++) {
		...
		auto xi = std::get<2>(_depthPositions[i]),
			 yj = std::get<1>(_depthPositions[i]),
			 zk = std::get<0>(_depthPositions[i]);
	...
	for (auto zk = 0; zk < Mz; zk++) {
		for (auto yj = 0; yj < My; yj++) {
			...
			if (_nSurf[specIdI()][yj][zk] > threshold) {
				...
				// Initialize the concentrations and the temperature on
				// the new grid points
