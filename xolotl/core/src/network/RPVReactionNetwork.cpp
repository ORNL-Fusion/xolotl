...
namespace network
{
template ReactionNetwork<RPVReactionNetwork>::ReactionNetwork(
...
template double
ReactionNetwork<RPVReactionNetwork>::getTotalConcentration(
	ConcentrationsView concentrations, Species type, AmountType minSize);

template double
ReactionNetwork<RPVReactionNetwork>::getTotalRadiusConcentration(
	ConcentrationsView concentrations, Species type, AmountType minSize);

template double
ReactionNetwork<RPVReactionNetwork>::getTotalAtomConcentration(
	ConcentrationsView concentrations, Species type, AmountType minSize);

template double
ReactionNetwork<RPVReactionNetwork>::getTotalTrappedAtomConcentration(
	ConcentrationsView concentrations, Species type, AmountType minSize);

template double
ReactionNetwork<RPVReactionNetwork>::getTotalVolumeFraction(
	ConcentrationsView concentrations, Species type, AmountType minSize);

double
RPVReactionNetwork::checkLatticeParameter(double latticeParameter)
{
...
double
RPVReactionNetwork::checkImpurityRadius(double impurityRadius)
{
...
RPVReactionNetwork::IndexType
RPVReactionNetwork::checkLargestClusterId()
{
	// Copy the cluster data for the parallel loop
	auto clData = _clusterData.d_view;
	using Reducer = Kokkos::MaxLoc<RPVReactionNetwork::AmountType,
		RPVReactionNetwork::IndexType>;
	Reducer::value_type maxLoc;
	Kokkos::parallel_reduce(
		"RPVReactionNetwork::checkLargestClusterId", _numClusters,
...
std::string
RPVReactionNetwork::getMonitorDataHeaderString() const
{
...
void
RPVReactionNetwork::addMonitorDataValues(Kokkos::View<const double*> conc,
...
void
RPVReactionNetwork::writeMonitorDataLine(
