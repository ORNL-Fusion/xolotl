 ...
 */
class VFitFluxHandler : public FluxHandler
{
private:
	/**
	 * \see FluxHandler.h
...
public:
	 ...
	 * The constructor
	 */
	VFitFluxHandler(const options::IOptions& options) : FluxHandler(options)
	{
	}
		...
		std::vector<double> grid) override
	{
		...
		using NetworkType = network::VReactionNetwork;
