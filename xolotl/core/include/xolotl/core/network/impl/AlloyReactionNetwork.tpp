...
AlloyReactionGenerator::operator()(IndexType i, IndexType j, TTag tag) const
{
	...
	if ((lo1.isOnAxis(Species::Void) && lo2.isOnAxis(Species::V)) ||
		...
		// Compute the composition of the new cluster
		auto size = lo1[Species::V] + lo2[Species::V] + lo1[Species::Void] +
			lo2[Species::Void]; // The other axis should be 0 so it should work
	...
	if ((lo1.isOnAxis(Species::Faulted) && lo2.isOnAxis(Species::V)) ||
		...
		// Compute the composition of the new cluster
		auto size = lo1[Species::V] + lo2[Species::V] + lo1[Species::Faulted] +
			lo2[Species::Faulted]; // The other axis should be 0 so it should
	...
	if ((lo1.isOnAxis(Species::Frank) && lo2.isOnAxis(Species::I)) ||
		...
		// Compute the composition of the new cluster
		auto size = lo1[Species::I] + lo2[Species::I] + lo1[Species::Frank] +
			lo2[Species::Frank]; // The other axis should be 0 so it should work
								 // to add up everything
		// Find the corresponding cluster
		Composition comp = Composition::zero();
		comp[Species::Frank] = size;
