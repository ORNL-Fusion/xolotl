...
public:
	...
	{
		return detail::CoefficientsView("Transform Coefficients", size,
	...
	computeConnectivity(const Connectivity& connectivity)
	{
		...
		for (auto i : speciesRangeNoI) {
			if (_productMomentIds[i()] != invalidIndex) {
				this->addConnectivity(_productMomentIds[i()],
					_productMomentIds[i()], connectivity);
			}
		}
	}
...
protected:
			...
			for (auto i : speciesRangeNoI) {
				if (_productMomentIds[i()] != invalidIndex) {
					this->addConnectivity(
						_productMomentIds[i()], _reactant, connectivity);
				}
			}
		}
	}
	...
	mapJacobianEntries(Connectivity connectivity)
	{
		...
		for (auto i : speciesRangeNoI) {
			if (_productMomentIds[i()] != invalidIndex) {
				_connEntries[1][1 + i()][0][0] = connectivity(
					_productMomentIds[i()], _productMomentIds[i()]);
			}
		}
	}
...
protected:
	...
	util::Array<IndexType, nMomentIds> _productMomentIds;

	util::Array<IndexType, 2, 1 + nMomentIds, 1, 1> _connEntries;
};
} // namespace network
} // namespace core
} // namespace xolotl
