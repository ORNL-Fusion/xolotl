...
void PetscSolver2DHandler::initializeConcentration(DM &da, Vec &C) {
	...
	temperatureHandler->updateSurfacePosition(grid[surfacePosition[0]+1] - grid[1]);
	...
	for (PetscInt j = ys; j < ys + ym; j++) {
		for (PetscInt i = xs; i < xs + xm; i++) {
			...
			// Temperature
			std::vector<double> gridPosition = { grid[i+1] - grid[1], 0.0, 0.0 };
			concOffset[dof - 1] = temperatureHandler->getTemperature(gridPosition,
					0.0);

			// Initialize the vacancy concentration
			if (i > surfacePosition[j] && vacancyIndex > 0 && !hasConcentrations) {
...
void PetscSolver2DHandler::updateConcentration(TS &ts, Vec &localC, Vec &F,
	...
	for (PetscInt yj = 0; yj < nY; yj++) {
		...
		// Loop over grid points
		for (int xi = surfacePosition[yj] + 1; xi < nX; xi++) {
		...
		advectionHandlers[0]->setLocation(grid[surfacePosition[yj]+1] - grid[1]);
		temperatureHandler->updateSurfacePosition(grid[surfacePosition[yj]+1] - grid[1]);
