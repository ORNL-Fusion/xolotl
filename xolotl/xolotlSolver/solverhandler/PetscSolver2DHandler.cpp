...
void PetscSolver2DHandler::createSolverContext(DM &da) {
	...
	mutationHandler->initialize(network, localXM, localYM);
...
void PetscSolver2DHandler::initGBLocation(DM &da, Vec &C) {
	...
	for (auto const& pair : gbVector) {
		...
		if (xi >= localXS && xi < localXS + localXM && yj >= localYS
			...
			setLocalXeRate(network.getTotalAtomConcentration(), xi - localXS, yj - localYS);
		...
		for (int xi = localXS; xi < localXS + localXM; xi++) {
			...
			if (xi == surfacePosition[yj]) {
				...
				temperatureHandler->computePartialsForTemperature(tempVals,
						tempIndices, hxLeft, hxRight, xi, sy, yj);
