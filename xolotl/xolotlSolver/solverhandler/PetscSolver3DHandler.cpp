...
void PetscSolver3DHandler::createSolverContext(DM &da) {
	...
	mutationHandler->initialize(expNetwork, dfill, xm, ym, zm);
	mutationHandler->initializeIndex3D(surfacePosition, expNetwork,
...
void PetscSolver3DHandler::initializeConcentration(DM &da, Vec &C) {
	...
	fluxHandler->initializeFluxHandler(expNetwork, surfacePosition[0][0], grid);
	...
	for (PetscInt k = zs; k < zs + zm; k++) {
		for (PetscInt j = ys; j < ys + ym; j++) {
			for (PetscInt i = xs; i < xs + xm; i++) {
				...
				concOffset[dof] = temperatureHandler->getTemperature(
						gridPosition, 0.0);
...
void PetscSolver3DHandler::updateConcentration(TS &ts, Vec &localC, Vec &F,
	...
	for (PetscInt zk = frontOffset; zk < nZ - backOffset; zk++) {
		for (PetscInt yj = bottomOffset; yj < nY - topOffset; yj++) {
			...
			fluxHandler->initializeFluxHandler(expNetwork,
			...
			for (PetscInt xi = xs; xi < xs + xm; xi++) {
				...
				// left
				double temperature = concs[zk][yj][xi - 1][dof];
				...
				// right
				temperature = concs[zk][yj][xi + 1][dof];
				if (std::fabs(lastTemperature[xi + 2 - xs] - temperature)
						> 0.1) {
				...
				temperature = temperatureHandler->getTemperature(gridPosition,
						ftime);
				// middle
				if (std::fabs(lastTemperature[xi + 1 - xs] - temperature)
						> 0.1) {
				...
				for (int i = 0; i < advectionHandlers.size(); i++) {
					advectionHandlers[i]->computeAdvection(expNetwork,
	...
	for (PetscInt zk = zs; zk < zs + zm; zk++) {
		...
		for (PetscInt yj = ys; yj < ys + ym; yj++) {
			...
			for (PetscInt xi = xs; xi < xs + xm; xi++) {
				...
				diffusionHandler->computePartialsForDiffusion(expNetwork,
						diffVals, diffIndices, hxLeft, hxRight, xi - xs, sy,
						yj - ys, sz, zk - zs);
				...
				gridPosition[0] = (grid[xi] + grid[xi + 1]) / 2.0 - grid[1];
				for (int l = 0; l < advectionHandlers.size(); l++) {
					advectionHandlers[l]->computePartialsForAdvection(
							expNetwork, advecVals, advecIndices, gridPosition,
							hxLeft, hxRight, xi - xs, hY, yj - ys, hZ, zk - zs);
	...
	for (PetscInt zk = frontOffset; zk < nZ - backOffset; zk++) {
		for (PetscInt yj = bottomOffset; yj < nY - topOffset; yj++) {
			...
			for (PetscInt xi = xs; xi < xs + xm; xi++) {
				...
				int nResoluting =
						resolutionHandler->computePartialsForReSolution(
								expNetwork, resolutionVals, resolutionIndices,
								xi, xs, yj, zk);
