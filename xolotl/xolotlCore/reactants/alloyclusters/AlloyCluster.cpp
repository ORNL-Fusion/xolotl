...
AlloyCluster::AlloyCluster() :
	...
	compositionMap[voidType] = 0;
	compositionMap[faultedType] = 0;
	compositionMap[frankType] = 0;
	compositionMap[perfectType] = 0;
...
AlloyCluster::AlloyCluster(AlloyCluster &other) :
	...
	compositionMap[voidType] = other.compositionMap[voidType];
	compositionMap[faultedType] = other.compositionMap[faultedType];
	compositionMap[frankType] = other.compositionMap[frankType];
	compositionMap[perfectType] = other.compositionMap[perfectType];
...
void AlloyCluster::createDissociation(
	...
	ClusterPair pair((AlloyCluster *) reaction->dissociating,
			(AlloyCluster *) emittedCluster);
...
void AlloyCluster::setReactionNetwork(
...
double AlloyCluster::getTotalFlux() {
	// Get the fluxes
...
double AlloyCluster::getDissociationFlux() const {
...
double AlloyCluster::getEmissionFlux() const {
	// Initial declarations
	int nPairs = 0;
	double flux = 0.0;
...
double AlloyCluster::getProductionFlux() const {
	// Local declarations
	double flux = 0.0;
	int nPairs = 0;
...
double AlloyCluster::getCombinationFlux() const {
	// Local declarations
	double flux = 0.0;
	int nReactants = 0;
...
std::vector<double> AlloyCluster::getPartialDerivatives() const {
	// Local Declarations
...
void AlloyCluster::getPartialDerivatives(std::vector<double> & partials) const {
	// Get the partial derivatives for each reaction type
	getProductionPartialDerivatives(partials);
	getCombinationPartialDerivatives(partials);
	getDissociationPartialDerivatives(partials);
...
void AlloyCluster::getCombinationPartialDerivatives(
	...
	for (int i = 0; i < numReactants; i++) {
		cluster = (AlloyCluster *) combiningReactants[i].combining;
...
void AlloyCluster::getEmissionPartialDerivatives(
		std::vector<double> & partials) const {
	// Initial declarations
	int numPairs = 0, index = 0;
...
void AlloyCluster::setDiffusionFactor(const double factor) {
...
void AlloyCluster::setMigrationEnergy(const double energy) {
...
double AlloyCluster::getLeftSideRate() const {
