...
inline void Reaction<TDerived>::computeProductionCoefficients(
    ...
    plsm::EnumIndexed<std::array<AmountType, numSpeciesNoI>, Species> width{};
    auto cl1Reg = network.getCluster(_reactants[0]).getRegion();
    auto cl2Reg = network.getCluster(_reactants[1]).getRegion();
    auto prodReg = network.getCluster(_products[0]).getRegion();
    ...
    for (auto i : speciesRangeNoI) {
        ...
        nOverlapProduct *= width[i];
    }

    // Verify that overlap is larger than 0
    assert(nOverlapProduct > 0);
    auto nOverlap = static_cast<double>(nOverlapProduct);
    ...
    for (auto i : speciesRangeNoI) {
        ...
        for (auto l : makeIntervalRange(cl2Reg[i])) {
            _coefs(i()+1, 0, 0, 0) += firstOrderSum(
                ...
                static_cast<double>(cl1Reg[i].end() - 1 + cl1Reg[i].begin()) / 2.0);
        }
        ...
        for (auto l : makeIntervalRange(cl1Reg[i])) {
            _coefs(0, i()+1, 0, 0) += firstOrderSum(
                ...
                static_cast<double>(cl2Reg[i].end() - 1 + cl2Reg[i].begin()) / 2.0);
        }
        ...
        for (auto l : makeIntervalRange(cl1Reg[i])) {
            _coefs(0, 0, 0, i()+1) += firstOrderSum(
                ...
                static_cast<double>(prodReg[i].end() - 1 + prodReg[i].begin()) / 2.0);
        }
            ...
            if (k == i) {
                for (auto l : makeIntervalRange(cl2Reg[i])) {
                    _coefs(i()+1, 0, 0, k()+1) += secondOrderOffsetSum(
                        std::max(prodReg[i].begin() - l, cl1Reg[i].begin()),
                        std::min(prodReg[i].end() - 1 - l, cl1Reg[i].end() - 1),
                        (double) (cl1Reg[i].end() - 1 + cl1Reg[i].begin()) / 2.0,
                        (double) (prodReg[i].end() - 1 + prodReg[i].begin()) / 2.0, l);
                }
                for (auto l : makeIntervalRange(cl1Reg[i])) {
                    _coefs(0, i()+1, 0, k()+1) += secondOrderOffsetSum(
                        std::max(prodReg[i].begin() - l, cl2Reg[i].begin()),
                        std::min(prodReg[i].end() - 1 - l, cl2Reg[i].end() - 1),
                        (double) (cl2Reg[i].end() - 1 + cl2Reg[i].begin()) / 2.0,
                        (double) (prodReg[i].end() - 1 + prodReg[i].begin()) / 2.0, l);
                }
            }
            else {
        ...
        for (auto j : speciesRangeNoI) {
            ...
            if (i == j) {
                for (auto l : makeIntervalRange(cl1Reg[j])) {
                    _coefs(i()+1, j()+1, 0, 0) += (l
                            - (double) (cl1Reg[j].end() - 1
                                    + cl1Reg[j].begin()) / 2.0)
                            * firstOrderSum(
                                    std::max(prodReg[j].begin() - l,
                                            cl2Reg[j].begin()),
                                    std::min(prodReg[j].end() - 1 - l,
                                            cl2Reg[j].end() - 1),
                                    (double) (cl2Reg[j].end() - 1
                                            + cl2Reg[j].begin()) / 2.0);
                }
            }
            else {
            ...
            for (auto k : speciesRangeNoI) {
                ...
                if (i == j && j == k) {
                    ...
                    _coefs(i()+1, j()+1, 0, k()+1) += _coefs(i()+1, 0, 0, 0)
                            * _coefs(0, j()+1, 0, k()+1) / nOverlap;
                }
                else if (i == k) {
                    _coefs(i()+1, j()+1, 0, k()+1) += _coefs(0, j()+1, 0, 0)
                            * _coefs(i()+1, 0, 0, k()+1) / nOverlap;
                }
                else {
                    // TODO check this is the right formula, might be divided by nOverlap^2
                    _coefs(i()+1, j()+1, 0, k()+1) += _coefs(i()+1, 0, 0, 0)
                            * _coefs(0, j()+1, 0, 0) * _coefs(0, 0, 0, k()+1)
                            / nOverlap;
                }
            }
            ...
            for (auto k : speciesRangeNoI) {
                ...
                if (i == j && j == k) {
                    ...
                    _coefs(i()+1, j()+1, 1, k()+1) += _coefs(0, j()+1, 0, 0)
                            * _coefs(i()+1, 0, 1, k()+1) / nOverlap;
                }
                else if (j == k) {
                    _coefs(i()+1, j()+1, 1, k()+1) += _coefs(i()+1, 0, 0, 0)
                            * _coefs(0, j()+1, 1, k()+1) / nOverlap;
                }
                else {
                    // TODO check this is the right formula, might be divided by nOverlap^2
                    _coefs(i()+1, j()+1, 1, k()+1) += _coefs(i()+1, 0, 0, 0)
                            * _coefs(0, j()+1, 0, 0) * _coefs(k()+1, 0, 0, 0)
                            / nOverlap;
                }
            }
            ...
            for (auto k : speciesRangeNoI) {
                ...
                if (i == j && j == k) {
                    ...
                    _coefs(i()+1, j()+1, 2, k()+1) += _coefs(0, j()+1, 0, 0)
                            * _coefs(i()+1, 0, 2, k()+1) / nOverlap;
                }
                else if (j == k) {
                    _coefs(i()+1, j()+1, 2, k()+1) += _coefs(i()+1, 0, 0, 0)
                            * _coefs(0, j()+1, 2, k()+1) / nOverlap;
                }
                else {
                    // TODO check this is the right formula, might be divided by nOverlap^2
                    _coefs(i()+1, j()+1, 2, k()+1) += _coefs(i()+1, 0, 0, 0)
                            * _coefs(0, j()+1, 0, 0) * _coefs(0, k()+1, 0, 0)
                            / nOverlap;
                }
            }
        }
...
inline void Reaction<TDerived>::computeDissociationCoefficients(
    ...
    auto prod1Reg = network.getCluster(_products[0]).getRegion();
    auto prod2Reg = network.getCluster(_products[1]).getRegion();
    AmountType nOverlapProduct = 1;
    // Loop on the phase space
    for (auto i : speciesRangeNoI) {
        // The width is the subset of the tiles for which the reaction is
        // possible
        // For instance if we have X_[5,7) -> X_1 + X_[3,5)
        // It is only possible for 4 within X_[3,5) and 5 within X_[5,7) so
        // the width is 1
        // More complicated with X_[9,11) -> X_[3,5) + X_[5,7)
        // 3+6, 4+5, 4+6, width is 3
        width[i] = 0;
        // Loop on the interval of product_1 (it would be great if it could
        // be the one with the smallest tile of the two)
        for (auto j : makeIntervalRange(prod1Reg[i])) {
            width[i] += std::min(clReg[i].end() - 1, prod2Reg[i].end() - 1 + j)
                    - std::max(clReg[i].begin(), prod2Reg[i].begin() + j) + 1;
        }

        nOverlapProduct *= width[i - 1];
    }
            ...
            if (k == i) {
                for (auto l : makeIntervalRange(prod2Reg[i])) {
                    _coefs(i()+1, 0, 1, k()+1) += secondOrderOffsetSum(
                            std::max(clReg[i].begin(),
                                    prod1Reg[i].begin() + l),
                            std::min(clReg[i].end() - 1,
                                    prod1Reg[i].end() - 1 + l),
                            (double) (clReg[i].end() - 1
                                    + clReg[i].begin()) / 2.0,
                            (double) (prod1Reg[i].end() - 1
                                    + prod1Reg[i].begin()) / 2.0,
                            -l);
                }
            }
            else {
            ...
            if (k == i) {
                for (auto l : makeIntervalRange(prod1Reg[i])) {
                    _coefs(i()+1, 0, 2, k()+1) += secondOrderOffsetSum(
                            ...
                            (double) (clReg[i].end() - 1
                                    + clReg[i].begin()) / 2.0,
                            (double) (prod2Reg[i].end() - 1
                                    + prod2Reg[i].begin()) / 2.0,
                            -l);
                }
            }
            else {
