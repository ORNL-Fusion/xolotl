#ifndef ONERESOLUTIONHANDLER_H
#define ONERESOLUTIONHANDLER_H
 ...
 * re-solution of xenon clusters on a one by one basis.
 */
class OneReSolutionHandler: public ReSolutionHandler {
...
public:
	...
	void initialize(const IReactionNetwork &network,
		...
		std::for_each(allClusters.begin(), allClusters.end(),
				[&network, this, &singleXenon, &y0, &a1, &b1, &b2, &c](
						IReactant &cluster) {
					// Get its size
					...
					if (resolutedSize > 0 && resolutedSize < size
						...
						if (cluster.getType() == ReactantType::NESuper) {
							...
							for (int i = numXe + 1.0
								...
								auto smallerCluster = network.get(Species::Xe,
										i - resolutedSize);
								// If it exists
								if (!smallerCluster) {
									...
									auto smallerSuper =
											static_cast<NESuperCluster*>(smallerCluster);
								...
								if (smallerCluster != previousSmaller
									...
									previousSmaller->setDissociationConnectivity(
											cluster.getMomentId());
									smallerSectionWidth =
											((NECluster*) previousSmaller)->getSectionWidth();
...
//end class OneReSolutionHandler
