...
public:
	 ...
	 *
	 * If D is the diffusion coefficient, and C_l, C_r, C_b, C_t, C_f, C_ba, C_m the
	 ...
	 *
	 * D * [sx * (C_l + C_r - 2*C_m) + sy * (C_b + C_t - 2*C_m)
	 *     + sz * (C_f + C_ba - 2*C_m)]
	 *
	 ...
	 * of the partial will be -2 * D * (sx + sy + sz).
	 *
	 * @param network The network
	...
	void computePartialsForDiffusion(PSIClusterReactionNetwork *network,
			double *val, int *indices, double sx, double sy = 0.0, double sz = 0.0);
