...
XolotlOptions::XolotlOptions(void) :
		...
		useTempProfileHandlers(false), usePerfStdHandlers(true), // by default, use "std" handlers for performance
		...
		useHeFlux(false),   // by default do not use the Helium flux option
		petscArgc(0), petscArgv( NULL), constTemp(1000)	// by default, the constant temperature is 1000K
{
	// Add our notion of which options we support.
	optionsMap["networkFile"] =
			new OptInfo(
					"networkFile                 The network will be loaded from this file.",
					handleNetworkOptionCB);
	optionsMap["startTemp"] =
			new OptInfo(
...
void XolotlOptions::readParams(int argc, char* argv[]) {
...
void XolotlOptions::showHelp(std::ostream& os) const {
	os << "usage: xolotl param_file_name \n\n"
			<< "See the Xolotl documentation for PETSc options. \n"
			<< "param_file_name should contain:" << std::endl;
	Options::showHelp(os);
}

bool XolotlOptions::handleNetworkOption(std::string arg) {
	bool ret = true;
...
bool XolotlOptions::handleNetworkOptionCB(Options* opts, std::string arg) {
	return static_cast<XolotlOptions*>(opts)->handleNetworkOption(arg);
}

bool XolotlOptions::handleConstTemperatureOption(std::string arg) {
	bool ret = true;
...
bool XolotlOptions::handleConstTemperatureOptionCB(Options* opts,
		std::string arg) {
	return static_cast<XolotlOptions*>(opts)->handleConstTemperatureOption(arg);
}

bool XolotlOptions::handleTemperatureFileOption(std::string arg) {
	bool ret = true;
...
bool XolotlOptions::handleTemperatureFileOptionCB(Options* opts,
		std::string arg) {
	return static_cast<XolotlOptions*>(opts)->handleTemperatureFileOption(arg);
}

bool XolotlOptions::handlePerfHandlersOption(std::string arg) {
	bool ret = true;
...
bool XolotlOptions::handlePerfHandlersOptionCB(Options* opts, std::string arg) {
	return static_cast<XolotlOptions*>(opts)->handlePerfHandlersOption(arg);
}

bool XolotlOptions::handleVizHandlersOption(std::string arg) {
	bool ret = true;
...
bool XolotlOptions::handleVizHandlersOptionCB(Options* opts, std::string arg) {
	return static_cast<XolotlOptions*>(opts)->handleVizHandlersOption(arg);
}

bool XolotlOptions::handleHeFluenceOption(std::string arg) {
	bool ret = true;
...
bool XolotlOptions::handleHeFluenceOptionCB(Options* opts, std::string arg) {
	return static_cast<XolotlOptions*>(opts)->handleHeFluenceOption(arg);
}

bool XolotlOptions::handleHeFluxOption(std::string arg) {
	bool ret = true;
	...
	useHeFlux = true;
	heliumFlux = strtod(arg.c_str(), NULL);

	return ret;
}

bool XolotlOptions::handleHeFluxOptionCB(Options* opts, std::string arg) {
	return static_cast<XolotlOptions*>(opts)->handleHeFluxOption(arg);
}

bool XolotlOptions::handlePetscOption(std::string arg) {
	bool ret = true;
	assert(!arg.empty());
...
bool XolotlOptions::handlePetscOptionCB(Options* opts, std::string arg) {
