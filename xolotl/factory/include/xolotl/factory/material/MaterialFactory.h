...
{
/**
 * Realizes the IMaterialFactory interface. Handles the flux, the advection,
 * modified trap mutation, and for a specific material.
 */
...
public:
	...
	MaterialFactory(const options::Options& opts)
	{
		...
		case 0:
			theDiffusionHandler =
				std::make_shared<core::diffusion::DummyDiffusionHandler>(
					threshold);
			break;
		case 1:
			theDiffusionHandler =
				std::make_shared<core::diffusion::Diffusion1DHandler>(
					threshold);
			break;
		case 2:
			theDiffusionHandler =
				std::make_shared<core::diffusion::Diffusion2DHandler>(
					threshold);
			break;
		case 3:
			theDiffusionHandler =
				std::make_shared<core::diffusion::Diffusion3DHandler>(
					threshold);
			break;
		default:
		...
		for (int i = 0; i < tokens.size(); i++) {
			...
			if (tokens[i] == "X") {
				...
				if (dim < 2)
					...
					throw std::string("\nA Y grain boundary CANNOT be used in "
									  "1D. Switch to 2D or 3D or remove it.");
				...
				if (dim < 3)
					...
					throw std::string("\nA Z grain boundary CANNOT be used in "
									  "1D/2D. Switch to 3D or remove it.");
