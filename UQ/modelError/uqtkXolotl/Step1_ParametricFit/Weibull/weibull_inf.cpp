...
main(int argc, char* argv[])
{
	...
	// Data noise standard deviation, either fixed, or initial position 	(if
	// inferred)
	...
	infer_model(forwardFunc, funcinfo, liktype, priortype, priora, priorb,
		...
		pdftype, pctype, seed, nmcmc, mcmcgamma, optimflag, chstart, chsig,
		likParam, likParam_int, pgrid, pchain, nburn, nstep, mapparam,
		datavar_map, pmean_map, pvar_map, fmean_map, fvar_map, postave_datavar,
...
forwardFunc(Array2D<double>& p, Array2D<double>& x, void* funcinfo)
{
	...
	for (int ip = 0; ip < np; ip++) {
		...
		for (int ix = 0; ix < nx; ix++)
			y(ip, ix) = p(ip, 0) / p(ip, 1) *
				pow(x(ix, 0) / p(ip, 1), (p(ip, 0) - 1.0)) *
				exp(-pow(x(ix, 0) / p(ip, 1), p(ip, 0)));
	}
	return y;
}
