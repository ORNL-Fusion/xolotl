...
RefPtr<XMLElement>
readXMLTree(const string inFileName);

void
readXMLChainInput(RefPtr<XMLElement> xmlTree, MCMC* pmchain,
...
void
readXMLModelInput(RefPtr<XMLElement> xmlTree, Array1D<double>& modelparams,
	Array1D<string>& modelparamnames, Array1D<double>& modelauxparams);
void
readXMLDataInput(RefPtr<XMLElement> xmlTree, Array2D<double>& data,
	Array1D<double>& postparams, string* noiseType);
void
readXMLUncInput(RefPtr<XMLElement> xmlTree, Array2D<double>& allPCcoefs,
	...
	Array1D<double>& postparams, string* noiseType)
{
	...
	*noiseType = dataInput->attributes()->get("noise", "const_std");
	...
	RefPtr<XMLAttributeList> postpar = dataInput->get_child("noise_types")
										   ->get_child(*noiseType)
										   ->attributes();
	...
	Array1D<int>& uncParamInd, int* outOrder, string* PCtype)
{
	...
	*outOrder = pcInput->attributes()->get_int("order", 1);
	...
	for (int i = 0; i < np; i++) {
		types(i) =
			params->get_child(i)->attributes()->get("type", "no_type_given");
	...
	for (int i = 0; i < np; i++) {
		double val =
			params->get_child(i)->attributes()->get_double("value", 1.0);
		allPCcoefs(0, i) = val;
	}
	...
	Array1D<double>& priorparam2)
{
	...
	string method =
		mcmcInput->attributes()->get("method", "no_method_specified");
	string chainparam =
		mcmcInput->attributes()->get("chainparam", "no_chainparam_specified");
	string output =
		mcmcInput->attributes()->get("output", "no_output_specified");
	cout << "Name       : " << name << endl;
	...
	RefPtr<XMLElement> chainparam_types =
		mcmcInput->get_child("chainparam_types");
	...
	if (!strcmp(chainparam.c_str(), "file")) {
		...
		read_datafileVS(
			chsig, pt2->attributes()->get("chsig", "chain_sig.dat").c_str());
	}
	else
		noFileFlag = true;
	...
	for (int i = 0; i < np; i++) {
		...
		if (!strcmp(type.c_str(), "infer")) {
			...
			if (noFileFlag) {
				...
				chsig.PushBack(params->get_child(i)->attributes()->get_double(
					"sigma", MAX(fabs(0.01 * val), 0.001)));
			}

			priortype.PushBack(
				params->get_child(i)->attributes()->get("prior", "uniform"));
			priorparam1.PushBack(
				params->get_child(i)->attributes()->get_double("pr1", 0.0));
			priorparam2.PushBack(
				params->get_child(i)->attributes()->get_double("pr2", 1000.0));
		}
	}
	...
	// \todo Need to set checks since the next three lines only useful for
	// method=am
