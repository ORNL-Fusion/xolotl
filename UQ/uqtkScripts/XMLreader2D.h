...
void readXMLChainInput(RefPtr<XMLElement> xmlTree, MCMC* pmchain,
		Array1D<double>& chstart, int* nsteps, Array1D<int>& chainParamInd,
		Array1D<string>& priortype, Array1D<double>& priorparam1,
		Array1D<double>& priorparam2);
void readXMLModelInput(RefPtr<XMLElement> xmlTree, Array1D<double>& modelparams,
		Array1D<string>& modelparamnames, Array1D<double>& modelauxparams);
void readXMLDataInput(RefPtr<XMLElement> xmlTree, Array2D<double>& data,
		Array1D<double>& postparams, string* noiseType);
void readXMLUncInput(RefPtr<XMLElement> xmlTree, Array2D<double>& allPCcoefs,
		Array1D<int>& uncParamInd, int* outOrder, string* PCtype);
...
// Compute the name of the input and write-back XML file
	...
	if (xmlLPos != string::npos) {
		...
		throw Tantrum(
				std::string("The XML input file ") + inFileName
	...
	if (!xmlInFile) {
		throw Tantrum(
				std::string("Could not open XML input file ") + inFileName);
	}
	ofstream xmlOutFile(outFileName.c_str(), ios::out);
	if (!xmlOutFile) {
		throw Tantrum(
				std::string("Could not open XML output file ") + outFileName);
	}
...
void readXMLDataInput(RefPtr<XMLElement> xmlTree, Array2D<double>& data,
	...
	*noiseType = dataInput->attributes()->get("noise", "const_std");
	...
	RefPtr < XMLAttributeList > postpar =
			dataInput->get_child("noise_types")->get_child(*noiseType)->attributes();
...
void readXMLModelInput(RefPtr<XMLElement> xmlTree, Array1D<double>& modelparams,
	...
	for (int i = 0; i < np; i++) {
		modelparams.PushBack(
				params->get_child(i)->attributes()->get_double("value", 0.e0));
		modelparamnames.PushBack(params->get_child(i)->label());
	}

// modelauxparams holds order and dim
	modelauxparams.Resize(2);
	modelauxparams(0) = params->attributes()->get_int("model_order_x", 1);
	modelauxparams(1) = params->attributes()->get_int("model_order_y", 1);
...
void readXMLUncInput(RefPtr<XMLElement> xmlTree, Array2D<double>& allPCcoefs,
	...
	*outOrder = pcInput->attributes()->get_int("order", 1);
	...
	for (int i = 0; i < np; i++) {
		types(i) = params->get_child(i)->attributes()->get("type",
				"no_type_given");
	...
	for (int i = 0; i < np; i++) {
		...
		double val = params->get_child(i)->attributes()->get_double("value",
				1.0);
		allPCcoefs(0, i) = val;
	}
		...
		for (int i = 0; i < np; i++) {
			...
			if (!strcmp(types(i).c_str(), "uncertain")) {
				for (int ip = 1; ip <= (*outOrder); ip++) {
					...
					mi(ic) = ip;
					allPCcoefs(get_invmindex(mi), i) =
							params->get_child(i)->attributes()->get_double(
									cf_str, 0.0);
				}
...
void readXMLChainInput(RefPtr<XMLElement> xmlTree, MCMC* pmchain,
	...
	string method = mcmcInput->attributes()->get("method",
			"no_method_specified");
	string chainparam = mcmcInput->attributes()->get("chainparam",
			"no_chainparam_specified");
	string output = mcmcInput->attributes()->get("output",
			"no_output_specified");
	cout << "Name       : " << name << endl;
	...
	RefPtr < XMLElement > chainparam_types = mcmcInput->get_child(
			"chainparam_types");
	...
	if (!strcmp(chainparam.c_str(), "file")) {
		...
		read_datafileVS(chstart,
				pt2->attributes()->get("chstart", "chain_start.dat").c_str());
		read_datafileVS(chsig,
				pt2->attributes()->get("chsig", "chain_sig.dat").c_str());
	} else
		noFileFlag = true;
	...
	for (int i = 0; i < np; i++) {
		...
		string type = params->get_child(i)->attributes()->get("type",
				"no_type_given");
		if (!strcmp(type.c_str(), "infer")) {
			chainParamInd.PushBack(i);
			if (noFileFlag) {
				double val = params->get_child(i)->attributes()->get_double(
						"value", 1.0);
				chstart.PushBack(val);
				chsig.PushBack(
						params->get_child(i)->attributes()->get_double("sigma",
								MAX(fabs(0.01 * val), 0.001)));
			}

			priortype.PushBack(
					params->get_child(i)->attributes()->get("prior",
							"uniform"));
			priorparam1.PushBack(
					params->get_child(i)->attributes()->get_double("pr1", 0.0));
			priorparam2.PushBack(
					params->get_child(i)->attributes()->get_double("pr2",
							1000.0));
		}
	}
