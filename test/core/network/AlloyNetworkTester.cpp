...
BOOST_AUTO_TEST_CASE(fullyRefined)
{
	...
	BOOST_CHECK_GT(deviceMemorySize, 90000);
	BOOST_CHECK_LT(deviceMemorySize, 100000);
...
BOOST_AUTO_TEST_CASE(grouped)
{
	...
	// TODO: check it is within a given range?
	auto deviceMemorySize = network.getDeviceMemorySize();
	BOOST_CHECK_GT(deviceMemorySize, 500000);
	BOOST_CHECK_LT(deviceMemorySize, 600000);
