...
BOOST_AUTO_TEST_CASE(checkModifiedTrapMutation)
{
	...
	trapMutationHandler.initializeIndex1D(
		surfacePos, network, advectionHandlers, grid, 13, 0);
	...
	trapMutationHandler.computeTrapMutation(
		network, concOffset, updatedConcOffset, 6);
	...
	trapMutationHandler.computeTrapMutation(
		network, concOffset, updatedConcOffset, 11);
	...
	BOOST_REQUIRE_CLOSE(
		updatedConcOffset[51], 9.53258e+20, 0.01); // Create He4V2
	...
	int nMutating = trapMutationHandler.computePartialsForTrapMutation(
		network, concOffset, valPointer, indicesPointer, 11);
